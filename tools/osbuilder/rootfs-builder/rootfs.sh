...
{
	local v1_str v2_str
	v1_str=$(echo "$1" | awk 'BEGIN {FS = "."} {print $1" "$2}')
	v2_str=$(echo "$2" | awk 'BEGIN {FS = "."} {print $1" "$2}')
	typeset -i -a v1
	read -ra v1 <<< "${v1_str}"
	typeset -i -a v2
	read -ra v2 <<< "${v2_str}"
...
EOF
	...
	if [[ -z "${AGENT_SOURCE_BIN}" ]] && [[ -z "${AGENT_TARBALL}" ]] ; then
		...
		if [[ "${SECCOMP}" == "yes" ]]; then
			...
			export libseccomp_install_dir
			gperf_install_dir=$(mktemp -d -t gperf.XXXXXXXXXX)
			export gperf_install_dir
		...
		if [[ -n "${AGENT_VERSION}" ]]; then
			if git checkout "${AGENT_VERSION}"; then
				OK "git checkout successful"
			else
				die "checkout agent ${AGENT_VERSION} failed!"
			fi
		fi
		make clean
