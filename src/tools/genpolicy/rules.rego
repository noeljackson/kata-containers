...
allow_mount(p_oci, i_mount, i_storages, bundle_id, sandbox_id):= p_index if {
    ...
    # check_mount expects a regex in the source field. Empty-source mounts are
    # handled only by the block-storage rule below.
    p_mount.source != ""
...
allow_mount(p_oci, i_mount, i_storages, bundle_id, sandbox_id):= p_index if {
    ...
    # This rule is exclusively for block-based emptyDir mounts, which don't
    # have a policy source.
    p_mount.source == ""
    some i_storage in i_storages
    print("allow_mount 2: i_storage =", i_storage)

    # Other storage drivers must not turn an empty policy source into an
    # attacker-selected guest path.
...
allow_storage(p_storages, i_storage, bundle_id, sandbox_id) if {
    ...
    # image_guest_pull storages always target the policy-owned rootfs path.
